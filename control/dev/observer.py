...
get_applied_ft = lambda b, fc, c, freqs: (get_ft(b, fc, c))(freqs)
...
def make_impulse(tt, N=100):
    ...
    c = stats.linregress(np.log10(freqs[np.where(freqs > 0.1)]), np.log10(clean_psd[np.where(freqs > 0.1)])).slope
    ft = lambda b, fc, c: lambda f: b/((1j * freqs + a)**(1/3) * (1j * freqs + fc)**(-c/2 - 1/3))
    def cost(pars):
        b, fc, c = pars
        return np.mean((np.log10(np.abs(get_applied_ft(b, fc, c, freqs))**2) - np.log10(P))**2)

    b, fc, c = optimize.minimize(cost, [1, 10, c]).x
    print(b, fc, c)
    ft = get_ft(b, fc, c)
    return design_filt(dt=1/f_sampling, N=N, tf = lambda f: 1/ft(f), plot=True)
