...
get_applied_ft = lambda b, fc, c: np.vectorize(get_ft(b, fc, c))(freqs)
...
def make_impulse(tt, N=100):
    ...
    def cost(pars):
        b, fc, c = pars
        return np.mean((np.log10(get_psd(get_applied_ft(b, fc, c))) - np.log10(P))**2)

    b, fc, c = optimize.minimize(cost, [1, 10, c]).x
    print(b, fc, c)
    ft = get_ft(b, fc, c)
    return design_filt(dt=1/f_sampling, N=N, tf = ft, plot=True)
