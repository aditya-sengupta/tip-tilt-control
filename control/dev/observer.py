...
def make_impulse(tt, N=20, plot=False):
    ...
    c1 = stats.linregress(np.log10(freqs[np.where(freqs[1:] <= 10)]), np.log10(clean_psd[np.where(freqs[1:] <= 10)])).slope
    ...
    def cost(pars):
        b, fc, c1, c2 = pars
        return np.mean((np.log10(np.abs(get_applied_ft(b, fc, c1, c2, freqs))**2) - np.log10(P))**2)
