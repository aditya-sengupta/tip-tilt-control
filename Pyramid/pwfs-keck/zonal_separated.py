		...
		for j in range(N):
			for ab in [1+0j, 0+1j]:
				...
				if np.complex(wfrefl[i][j]) != 0:
					l[i][j] = ab
					basis_element = Wavefront(Field(np.round(np.asarray(l).ravel(), 3) * aperture(pupil_grid), wfref.grid))
					pyramid_basis.append(make_slopes(basis_element))
	return np.asarray(pyramid_basis)

def main():
	pyb = make_pyramid_basis()
	print(pyb.shape)
	pyb.tofile('zonal.dat')
	print("Done.")
